...
for /f "usebackq delims=" %%I in (`powershell -NoProfile -Command "Get-Date -Format 'yyyyMMdd-HHmmss'" 2^>nul`) do set "timestamp=%%I"
if not defined timestamp set "timestamp=%RANDOM%_%RANDOM%"
set "outputFile=%~dp0SystemInfo_%timestamp%.txt"
set "DXTEMP=%TEMP%\dxtemp_%RANDOM%_%RANDOM%.txt"
...
:: Storage Information
:: Win32_DiskDrive.InterfaceType often reports IDE for SATA/AHCI disks; BusType from Get-PhysicalDisk is the actual interconnect.
call powershell -Command "$disks = Get-CimInstance -ClassName Win32_DiskDrive; $physicalDisks = @(); try { $physicalDisks = @(Get-PhysicalDisk -ErrorAction Stop) } catch { Write-Warning ('Get-PhysicalDisk failed: ' + $_.Exception.Message) }; '=== Storage Information ===' | Out-File -FilePath '%outputFile%' -Append -Encoding UTF8; foreach ($disk in $disks) { $diskSerial = if ($disk.SerialNumber) { $disk.SerialNumber.Trim() } else { '' }; $physical = $physicalDisks | Where-Object { $_.DeviceId -eq ([string]$disk.Index) -or ($diskSerial -and $_.SerialNumber -and ($_.SerialNumber.Trim() -eq $diskSerial)) } | Select-Object -First 1; $interfaceType = if ($physical -and $physical.BusType) { [string]$physical.BusType } else { $disk.InterfaceType }; $mediaType = if ($physical -and $physical.MediaType) { [string]$physical.MediaType } else { $disk.MediaType }; 'Disk Drive:' | Out-File -FilePath '%outputFile%' -Append -Encoding UTF8; '  Model: ' + $disk.Model | Out-File -FilePath '%outputFile%' -Append -Encoding UTF8; '  Size (GB): ' + [math]::Round($disk.Size/1GB, 2) | Out-File -FilePath '%outputFile%' -Append -Encoding UTF8; '  Interface Type: ' + $interfaceType | Out-File -FilePath '%outputFile%' -Append -Encoding UTF8; '  Media Type: ' + $mediaType | Out-File -FilePath '%outputFile%' -Append -Encoding UTF8; '  Partitions: ' + $disk.Partitions | Out-File -FilePath '%outputFile%' -Append -Encoding UTF8 }; '' | Out-File -FilePath '%outputFile%' -Append -Encoding UTF8"
